    ...
    <Surface className="p-2">
      <form onSubmit={state.handleSubmit} className="flex items-center gap-2">
        <label className="flex items-center gap-2 p-2 rounded-lg bg-neutral-100 dark:bg-neutral-900 cursor-text">
          ...
          <input
            type="url"
            className="flex-1 bg-transparent outline-hidden min-w-48 text-black text-sm dark:text-white"
            placeholder="Enter URL"
            value={state.url}
            onChange={state.onChange}
          />
        </label>
        <Button variant="secondary" type="submit" isDisabled={!state.isValidUrl}>
          Set Link
        </Button>
      </form>
