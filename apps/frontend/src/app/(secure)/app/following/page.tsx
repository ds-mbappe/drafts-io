...
import { useInfiniteFollowingDrafts } from '@/hooks/useDraft';
...
const FollowingPage = () => {
  const { items, hasMore, loadMore, isLoading, isValidating } = useInfiniteFollowingDrafts();
    ...
    <div className="w-full flex flex-col gap-5 mx-auto relative py-10 px-5">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-foreground-500">
          Latest drafts from people you follow.
        </p>
      </div>
      ...
      {isLoading && items.length === 0 ? (
        ...
        <p className="text-sm text-foreground-500">
          No drafts from people you follow yet.
        </p>
      )}
    </div>
  );
